...
void multiplyOriginal(int n, int32_t input[n][n], int32_t output[n][n]) {
    for(int i = 0; i < n; i++) {
        for(int k = 0; k < n; k++) {
            for(int j = 0; j < n; j++) {
...
void multiply(int n, int32_t input[n][n], int32_t output[n][n]) {
    
    int32_t transpose[n][n];
    ...
    for(int i = 0; i < n; i++) {
        ...
        for(int j = 0; j < n; j++) {
            ...
            for(; k + 8 < n; k += 8) {
                __m256i row = _mm256_loadu_si256((__m256i*)&input[i][k]);
                __m256i col = _mm256_loadu_si256((__m256i*)&transpose[j][k]);
            ...
            for(; k < n; k++) {
                tempSum += input[i][k] * transpose[j][k];
            }
